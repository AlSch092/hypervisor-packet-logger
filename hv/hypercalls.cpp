...
void register_custom_task(vcpu* const cpu) {
    auto const pid = cpu->ctx->rcx; //uint64_t
    auto const mode = cpu->ctx->rdx; //uint8_t
	auto const extra_data = cpu->ctx->r8; //uint64_t
    auto const enable = cpu->ctx->r9; //boolean

    bool result = custom_tasks::register_task(pid, (custom_tasks::task_code)mode, extra_data,(uint8_t)enable);
    cpu->ctx->rax = result;
    skip_instruction();
}

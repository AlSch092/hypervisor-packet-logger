...
namespace hv {
	...
	void emulate_vmcall(vcpu* const cpu) {
		...
		case hypercall_register_custom_task: hc::register_custom_task(cpu); return;
	...
	void handle_ept_violation(vcpu* const cpu) {
		...
		for (auto const& entry : cpu->ept.mmr) {
			...
			if (custom_tasks::get_tasks_count() > 0) {

				auto const _pid = current_guest_pid();
				auto const rip = vmx_vmread(VMCS_GUEST_RIP);

				for (uint8_t i = 0; i < custom_tasks::max_tasks; ++i)
				{
					custom_tasks::custom_task task = custom_tasks::get_task(i);

					if (task.pid == _pid && task.extra_data == rip)
					{
						executed_task = true;
						...
						case  custom_tasks::log_send_packets:
						{
							...
							const uint64_t target_pid_cr3 = get_cr3(_pid);
							...
							size_t bytes_read = hv::read_guest_virtual_memory(guest_cr3, (void*)data_buffer, (void*)buffer, data_size);
							
							if (bytes_read)
							{
								HV_LOG_MMR_ACCESS("    [INFO] READ send() from pid=%d, data of size:  %d", _pid, bytes_read);
			...
			if (!executed_task && is_relevant_mode &&

...
enum hypercall_code : uint64_t {
  ...
  hypercall_register_custom_task,
};
...
inline void register_custom_task(const uint32_t pid, const uint8_t mode, const uint64_t extra_data, const bool enable) {
    hv::hypercall_input input;
    input.code = hv::hypercall_register_custom_task;
    input.key  = hv::hypercall_key;
    input.args[0] = pid;
    input.args[1] = mode;
    input.args[2] = extra_data;
    input.args[3] = enable;
    hv::vmx_vmcall(input);
}
